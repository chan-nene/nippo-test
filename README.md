...
`storage.example.ini` は配布用の設定例です。実際の `storage.ini` と `settings.ini` は環境・個人設定を含むためGit管理しません。`settings.ini` は初回起動時にアプリと同じフォルダへ既定値で自動作成され、表示や未コメント集計などの個人/UI設定だけを保持します。保存先の `root_path` は保存しません。

同じ配置フォルダの多重起動は、`cache/locks/daily-report.lock` のOSファイルロックで防止します。2回目はメッセージ表示や既存ウィンドウのアクティブ化を行わずに終了します。別フォルダのアプリは同時起動できます。ロック用にAppDataなどアプリフォルダ外へファイルを作成しません。異常終了時もOSがロックを解放します。

IME診断はPythonでの開発実行時のみ有効です。PyInstallerのexe実行時（`sys.frozen`）は画面の診断収集・送信とAPIのログ記録を無効化し、WebViewのデバッグモードも無効にします。
...
共通マスタは上記の現行列構成だけを読み込みます。旧組織階層名（`large`・`medium`・`small`）、旧ユーザー・コメント担当形式、旧カレンダー形式の互換読み込みと移行機能はありません。列構成が一致しないファイルは読み込み警告の対象となり、自動変換や上書きは行いません。

管理画面の稼働日カレンダーは4月1日から翌年3月31日までを1年度として表示します。`calendar.csv` は休日の日付だけを保持し、行がない平日は稼働日として扱います。
